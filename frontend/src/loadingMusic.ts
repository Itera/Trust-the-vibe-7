...
export function startLoadingMusic() {
  // Cancel any pending stop-timer from a previous invocation (React StrictMode
  // runs effects twice: start → stop → start; without this the stop-timer from
  // the first cycle closes the AudioContext created by the second cycle).
  if (stopTimer) { clearTimeout(stopTimer); stopTimer = null; }
  if (startTimer) { clearTimeout(startTimer); startTimer = null; }

  // Close the previous context cleanly, if any.
  ...
  barCount = 0;
  // Small lookahead so the first bar isn't scheduled before the context resumes
  startTimer = setTimeout(() => { startTimer = null; scheduleBar(); }, 50);
...
export function stopLoadingMusic() {
  ...
  // Capture current references so a subsequent startLoadingMusic() can't
  // accidentally have its context closed by this timeout.
  const ctxToClose = ctx;
  stopTimer = setTimeout(() => {
    ctxToClose?.close();
    if (ctx === ctxToClose) { ctx = null; master = null; }
    stopTimer = null;
  }, 400);
}
